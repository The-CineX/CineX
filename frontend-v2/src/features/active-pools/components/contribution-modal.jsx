      ...
      <div className="relative w-full max-w-xl mx-4 bg-gradient-radial-dark border border-gray-900/40 rounded-2xl p-6">
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            <p className="text-gray-400 text-sm">Goal: ${pool.fundingGoal.toLocaleString()} — Remaining: ${remaining.toLocaleString()}</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm text-gray-300 mb-2">Contribution Amount (USD)</label>
            <input
              type="number"
              value={amount}
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button type="submit" className="flex-1 px-4 py-3 bg-yellow-400 text-black font-bold rounded-lg">Contribute</button>
          </div>
        </form>
      </div>
    </div>
